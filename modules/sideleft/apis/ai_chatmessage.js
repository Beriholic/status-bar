...
const CUSTOM_SOURCEVIEW_SCHEME_PATH = `${
  App.configDir
}/assets/themes/sourceviewtheme${darkMode.value ? "" : "-light"}.xml`;
...
cd /opt/MicroTeX
./LaTeX -headless -input="$text" -output=${outFilePath} -textsize=${
          fontSize * 1.1
        } -padding=0 -maxwidth=${
          latexViewArea.get_allocated_width() * 0.85
        } > /dev/null 2>&1
sed -i 's/fill="rgb(0%, 0%, 0%)"/style="fill:#000000"/g' ${outFilePath}
sed -i 's/stroke="rgb(0%, 0%, 0%)"/stroke="${
          darkMode.value ? "#ffffff" : "#000000"
        }"/g' ${outFilePath}
`;
    ...
    attribute: {
      fullUpdate: (self, content, useCursor = false) => {
        ...
        if (lastProcessed < lines.length) {
          ...
          if (!inCode)
            lastLabel.label = `${md2pango(blockContent)}${
              useCursor ? userOptions.ai.writingCursor : ""
            }`;
      ...
      Box({
        ...
        children: [
          Label({
            hpack: "start",
            xalign: 0,
            className: `txt txt-bold sidebar-chat-name sidebar-chat-name-${
      ...
      Box({
        ...
        children: [
          Label({
            ...
            hpack: "start",
            className:
              "txt txt-bold sidebar-chat-name sidebar-chat-name-system",
            wrap: true,
