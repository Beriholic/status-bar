...
export const waifuTabIcon = Box({
  hpack: "center",
  children: [MaterialIcon("photo", "norm")],
});
            ...
            ImageAction({
              ...
              action: (self) => {
                execAsync([
                  ...
                  } $(xdg-user-dir PICTURES)/homework${
                    thisBlock.attribute.isNsfw ? "/🌶️/" : ""
                  }`,
                ])
      ...
      update: (imageData, force = false) => {
        ...
        if (!force && fileExists(thisBlock.attribute.imagePath)) showImage();
          ...
          Utils.execAsync([
            ...
            .then(showImage)
            .catch(print);
        thisBlock.css = `background-color: mix(${
          darkMode.value ? "black" : "white"
        }, ${dominant_color}, 0.97);`;
      },
    },
    children: [
...
function newSimpleImageCall(
  name,
  url,
  width,
  height,
  dominantColor = "#9392A6"
) {
