...
export const AnimatedSlider = ({ className, value, ...rest }) => {
  return Widget.DrawingArea({
    className: `${className}`,
    setup: (self) => {
      self.connect(
        "draw",
        Lang.bind(self, (self, cr) => {
          ...
          const minWidth = styleContext.get_property(
            "min-width",
          ...
          const radius = styleContext.get_property(
            "border-radius",
            Gtk.StateFlags.NORMAL
          );
          const bg = styleContext.get_property(
            "background-color",
            Gtk.StateFlags.NORMAL
          );
          const fg = styleContext.get_property("color", Gtk.StateFlags.NORMAL);
          const value =
            styleContext.get_property("font-size", Gtk.StateFlags.NORMAL) / 100;
