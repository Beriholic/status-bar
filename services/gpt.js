...
class GPTService extends Service {
  ...
  _messages = [];
  _key = "";
  _key_file_location = `${GLib.get_user_state_dir()}/ags/user/ai/${
    PROVIDERS[this._currentProvider]["key_file"]
  }`;
  _url = GLib.Uri.parse(
  ...
  _initChecks() {
    this._key_file_location = `${GLib.get_user_state_dir()}/ags/user/ai/${
      PROVIDERS[this._currentProvider]["key_file"]
    }`;
